<?php
	...
	function edit_members_form()
	{
		...
		$form->addElement(new XoopsFormText('Display Name:', "display_name", 35, 128, $display_name));
		$form->addElement(new GenobioFormSelectSex('Sex:', "member_sex", $member_sex, 1, false));		
		$form->addElement(new GenobioFormSelectDisplayPicture('Display Name:', "display_picture", $display_picture, 1, false));		
		$form->addElement(new GenobioFormSelectCategory('Category:', "category_id", $category_id, 1, false));
		$form->addElement(new XoopsFormSelectUser('Username:', "uid", true, $uid, 1, false));
		if (class_exists('XoopsFormSelectDomains')) {
			$form->addElement(new XoopsFormSelectDomains('Personal Domain:', "domain", $domain, 1, false));
			$form->addElement(new XoopsFormSelectDomains('Associated Domain:', "domains", $domains, 8, true));
		} else {
